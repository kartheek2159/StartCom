  ...
  mongoClient.connect("mongodb://localhost:27017", function (err, client) {
    ...
    app.post("/admin", async (req, res) => {
      if (req.fields.userrname) {
        var myquery = { _id: ObjectId(req.fields.userrname) };
        database.collection("users").deleteOne(myquery, function (err, obj) {
          if (err) throw err;
      ...
      if (req.fields.posts) {
        var myquery = { _id: ObjectId(req.fields.posts) };
        database.collection("posts").deleteOne(myquery, function (err, obj) {
          if (err) throw err;
          console.log("1 document deleted");
          res.redirect("/admin");
        });
      ...
      if (req.fields.pages) {
        var myquery = { _id: ObjectId(req.fields.pages) };
        database.collection("pages").deleteOne(myquery, function (err, obj) {
          if (err) throw err;
          console.log("1 document deleted");
          res.redirect("/admin");
        });
      }
      if (req.fields.groups) {
        var myquery = { _id: ObjectId(req.fields.groups) };
        database.collection("groups").deleteOne(myquery, function (err, obj) {
          if (err) throw err;
          console.log("1 document deleted");
          res.redirect("/admin");
        });
